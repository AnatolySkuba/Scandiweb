							...
							<div key={item.item.id}>
								<CartItemContainer>
									<CartItemDetailsContainer>
										...
										<ProductPrice>{`${item.item.prices[currency].currency.symbol} ${item.item.prices[currency].amount}`}</ProductPrice>
										<Attributes
											item={item.item}
									...
									<CartItemActionsContainer>
										<CartCountButton value={item} onClick={() => this.handleIncrease({ item })}>
											+
										</CartCountButton>
										<h3>{item.quantity}</h3>
										<CartCountButton value={item} onClick={() => this.handleRemove({ item })}>
											-
										</CartCountButton>
									</CartItemActionsContainer>
